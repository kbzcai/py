...
class ReadS71200(object):
    def __init__(self, ip):
        ...
        self.cursor = None
        self.ip = ip
        self.siemens = SiemensS7Net(SiemensPLCS.S1200, self.ip)
        if not self.siemens.ConnectServer().IsSuccess:
            print("PLC连接失败")
        else:
            print("PLC连接成功")

    def sql_connect(self, user, password, db_name):
        self.db = pymysql.connect(user, password, db_name)
...
if __name__ == "__main__":
    ...
    s7.sql_connect("root", "rootroot", "mes")
    s7.read_data()
