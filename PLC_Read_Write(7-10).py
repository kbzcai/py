...
sys.path.append(r"D:\py\venv\Lib\site-packages")
import time
...
class ReadS71200(object):
    ...
    def plc_connect(self, plc_ip1, plc_ip2, plc_ip3, plc_ip4):
        try:
            global siemens1, siemens2, siemens3, siemens4
            if self.state1[0] == 0:
    ...
    def main(self):
        if self.state1[0] == 1:
            self.insertDataDB(siemens1, "mes_plc_r7")
        if self.state1[1] == 1:
            self.insertDataDB(siemens2, "mes_plc_r8")
        if self.state1[2] == 1:
            self.insertDataDB(siemens3, "mes_plc_r9")
        if self.state1[3] == 1:
            self.insertDataDB(siemens4, "mes_plc_r10")
...
def plc_read():
    global plc_data
    plc_data = ReadS71200("192.168.1.102", "root", "123456", "mes")
...
def data_json(data):
    data_list = {"productType": data["product_type"], "r1EquipmentStatus": data["r1_equipment_status"],
                 "failCode": data["fail_code"], "prodNum": data["prod_num"], "r1RobNo": data["r1_rob_no"],
                 "r1WeldLineNo": data["r1_weld_line_no"], "r1Electric": data["r1_electric"],
                 "r1Voltage": data["r1_voltage"],
                 "r1Speed": data["r1_speed"], "r1Job": data["r1_job"], "r2RobNo": data["r2_rob_no"],
                 "r2WeldLineNo": data["r2_weld_line_no"],
                 "r2Electric": data["r2_electric"], "r2Voltage": data["r2_voltage"],
                 "r2Speed": data["r2_speed"],
                 "r2Job": data["r2_job"], "replaceStatus": data["replace_status"],
                 "r1ProtectiveGas": data["r1_protective_gas"],
                 "r2ProtectiveGas": data["r2_protective_gas"], "r1WeldingWire": data["r1_welding_wire"],
                 "r2WeldingWire": data["r2_welding_wire"], "loginId": data["login_id"],
                 "loginPassword": data["login_password"],
                 "partStatus": data["part_status"], "recerve": data["recerve"],
...
def readdataplc(num):
    ...
    try:
        ...
        if num == 0:
            ...
            data_list = {"1": data_list1, "2": data_list2, "3": data_list3, "4": data_list4}
        elif num == 1:
